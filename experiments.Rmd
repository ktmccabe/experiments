...
In each case, we standardize our estimates according to the student's t-distribution. We look to see just how extreme our t statistic is.

### p-values


What is the probability of getting a result this extreme or more extreme "by chance"/"if the null were true"?
...
We primarily use two-sided tests. To get the p-value, we need the degrees of freedom. For one sample tests, it is N-1. For two-sample, it can be more complicated. 

If we use the Welch calculation for unequal variances, it is: df$=\frac{\widehat{SE}^4}{ \frac{\widehat{Var}(Y_i(1))^2}{m^2(m-1)} + {\frac{\widehat{Var}(Y_i(0))^2}{(N-m)^2(N-m-1)}}}$.
...
When learning a new function, you can access the help files in R by typing `?FUN` into the console. Example: `t.test`.
...
What is our average treatment effect for \textcolor{blue}{$d$}? 18/4 - 15/3 = -.5
...
probs <- genprobexact(asians$treatment_cit)
ate <- estate(asians$study2_avg,asians$treatment_cit,prob=probs)

## Generate permutations of random assignments
perms <- genperms(asians$treatment_cit,maxiter=1000) 

## Generate Potential Outcomes. When null ate = 0, this will just be your Y
Ys <- genouts(asians$study2_avg,asians$treatment_cit,ate=0)

## Simulate the implied distribution of ates
...
```{r}
results <- dispdist(distout, ate)
```

```{r}
## Two-tailed p-value
results$two.tailed.p.value
2*length(distout[distout >= ate])/length(distout)

results$two.tailed.p.value.abs
length(distout[abs(distout) >= ate])/length(distout)
```
...
ANOVA and t-test diverge in estimates when you have more than 2 groups
  ...
  - As there are multiple-testing corrections for t-tests
      + More on [multiple testing adjustments](https://egap.org/methods-guides/10-things-you-need-know-about-multiple-comparisons)
